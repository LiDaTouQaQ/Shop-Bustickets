...
<body>
  ...
  <div class="counter">
    ...
    <script>
		...
		<div class="do_box">
				<div>
					<span>用户名</span> <span><%=request.getAttribute("user")%></span>
				</div>
				<div>
					<span>出发地-目的地</span> <span><%=request.getAttribute("city")%></span>
				</div>
				<div>
					<span>出发日期和时间</span> <span><%=request.getAttribute("time")%></span>
				</div>
				<div>
					<span>车票单价</span> <span><%=request.getAttribute("price")%></span>
				</div>
				<div>
					<span>手机号码</span> <span><%=request.getAttribute("tel")%></span>
				</div>
				<div>
					<span>票数</span> <span><input value="1" type="number"></span>
				</div>
				<div>
					<span>总价</span> <span>0</span>
				</div>
			<form action="Confirm" method="post" encoding="">
				<input type="text" name="id"
					value="<%=request.getAttribute("orderid")%>" style="display: none;">
				<input type="text" name="city"
					value="<%=request.getAttribute("city")%>" style="display: none;">
				<input type="text" name="station"
					value="<%=request.getAttribute("station")%>" style="display: none;">
				<input type="text" name="time"
					value="<%=request.getAttribute("time")%>" style="display: none;">
				<input type="text" name="costtime"
					value="<%=request.getAttribute("costtime")%>"
					style="display: none;"> 
				<input type="text" name="price"
					value="<%=request.getAttribute("price")%>" style="display: none;">
				<input type="text" name="user"
					value="<%=request.getAttribute("user")%>" style="display: none;">
				<input type="text" name="tel"
					value="<%=request.getAttribute("tel")%>" style="display: none;">
				<input type="text" name="num" value="" style="display: none;">
				
				<input type="submit" value="确认订票">
			</form>
		</div>
			<script>
				const td6 = document.querySelector('.do_box div:nth-child(6) span input')
				const td7 = document.querySelector('.do_box div:nth-child(7) span:last-child')
				const td4 = document.querySelector('.do_box div:nth-child(4) span:last-child')
				td6.addEventListener('mouseleave', function() {
					td7.textContent = Number(td6.value)* Number(td4.textContent) + "￥"
					const num = document.querySelector('form input:nth-child(9)')
					num.value = td6.value
